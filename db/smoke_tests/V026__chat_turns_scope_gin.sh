...
# V026.1: index exists with the right method
...
echo "$HAS" | grep -qi "USING gin (access_scope_ids)" \
  && pass "V026.1" "chat_turns_access_scope_idx exists as GIN(access_scope_ids)" \
  || fail "V026.1" "index missing or wrong method: '$HAS'"

# V026.2: array-containment query returns hits + planner accepts it
...
[ "$COUNT" = "1" ] \
  && pass "V026.2" "@> containment returns turns whose scope touched a given org (count=$COUNT)" \
  || fail "V026.2" "expected 1 turn touching OU_OTHER, got $COUNT"

# V026.3: empty array doesn't match a non-empty filter
COUNT=$(sql "SELECT COUNT(*) FROM chat_turns
             WHERE access_scope_ids @> ARRAY['dead0026-0000-0000-0000-000000000000'::uuid];")
[ "$COUNT" = "0" ] \
  && pass "V026.3" "non-matching containment returns 0 turns" \
  || fail "V026.3" "expected 0 for non-matching scope, got $COUNT"
