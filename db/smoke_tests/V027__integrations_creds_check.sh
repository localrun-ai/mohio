...
# V027.1: both NULL accepted (integration exists, credentials not yet configured)
...
[ "$COUNT" = "1" ] \
  && pass "V027.1" "both NULL accepted (no credentials yet)" \
  || fail "V027.1" "both-NULL rejected (count=$COUNT)"

# V027.2: both set accepted (credentials encrypted with a known key)
sql "INSERT INTO integrations
       (id, company_id, org_unit_id, type, name, credentials, credentials_key_id)
...
[ "$COUNT" = "1" ] \
  && pass "V027.2" "both-set accepted (credentials+key_id together)" \
  || fail "V027.2" "both-set rejected (count=$COUNT)"

# V027.3: creds set + key_id NULL rejected (orphan ciphertext)
...
echo "$ERR" | grep -qi "check\|violates" \
  && pass "V027.3" "creds set + key_id NULL rejected (orphan ciphertext blocked)" \
  || fail "V027.3" "orphan ciphertext not rejected: $ERR"

# V027.4: creds NULL + key_id set rejected (dangling key reference)
ERR=$(sql "INSERT INTO integrations
             (id, company_id, org_unit_id, type, name, credentials, credentials_key_id)
           VALUES ('$INT_BAD2','$CO_ACME','$ACME_ROOT','slack','slack-bad2',
                   NULL, 'key-v1');" 2>&1 || true)
echo "$ERR" | grep -qi "check\|violates" \
  && pass "V027.4" "creds NULL + key_id set rejected (dangling key reference)" \
  || fail "V027.4" "dangling key reference not rejected: $ERR"

# V027.5: UPDATE path also enforced (clearing only one side rejected)
ERR=$(sql "UPDATE integrations SET credentials = NULL WHERE id='$INT_OK_BOTH';" 2>&1 || true)
echo "$ERR" | grep -qi "check\|violates" \
  && pass "V027.5" "UPDATE that clears only one side rejected" \
  || fail "V027.5" "asymmetric UPDATE not rejected: $ERR"
